...
@jsx.component
let make = (~player) =>
  switch Preact.get(`.ytp-popup.ytp-settings-menu .ytp-panel .ytp-panel-menu`) {
  | Value(ytSettingsMenu) => {
      ...
      <>
        ...
        >
          <CcIcon
            opacity={Signalish.fromSignal(
              Signal.useComputed(() => anyCaptions.value ? "1.0" : "0.3"),
            )}
          />
        </button>
