    ...
    class baseSubtitles {
      ...
      constructor(subId, checked, text, ...par) {
        ...
        if (!sub.has(subId)) {
          ...
          this.checkbox.addEventListener('click', ()=>this.clickCheckbox());
      ...
      clickCheckbox(){
        this.track.mode=this.isChecked()?"showing":'hidden';
      }
      ...
      partitionIntoSegments() {
        ...
        this.track=videoTag.addTextTrack("captions", this.subId, this.subId);
        ...
        this.lines.forEach(({ start, end }, index) => {
          ...
          this.track.addCue(new VTTCue(rStart, rStart+.1, this.lines[index].html));
        });
...
ytd-rich-item-renderer { margin-bottom: 0 !important }
...
a.ytp-subtitles-button.ytp-button[aria-pressed="true"] svg {
    fill-opacity: 1;
}`;
      }
    }
